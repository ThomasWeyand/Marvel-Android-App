...
import br.com.thomas.weyandmarvel.db.CharacterOffset
...
class CharacterRemoteMediator(
    ...
    override suspend fun load(
        ...
        val apiResult = repositoryService.getCharactersList(offset = pageOffet)
            ...
            is Result.Success -> {
                ...
                database.withTransaction {
                    ...
                    if (loadType == LoadType.REFRESH) {
                        database.offsetDao().clearRemoteOffset()
                    }
                    ...
                    database.offsetDao().insertAll(offsetValues)
    ...
    private suspend fun getOffsetValueForLastItem(state: PagingState<Int, ResultItem.Character>): CharacterOffset? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { character ->
                database.offsetDao().getOffsetById(character.id)
            }
    }

    private suspend fun getOffsetValueForFirstItem(state: PagingState<Int, ResultItem.Character>): CharacterOffset? {
    ...
    private suspend fun getOffsetValueClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { characterId ->
                database.offsetDao().getOffsetById(characterId)
            }
        }
    }
